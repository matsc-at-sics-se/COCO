...
## argument 2: the chromsomes of interest
## argument 3: the value to scale the interaction frequency by (in order to convert it to
## an integer
## argument 4: start index of the chromsome in the interaction matrix
## argument 5: stop index of the chromsome in the interaction matrix
## argument 6: size of the chromosome (number of bins)
##
...
for(my $row = $chr_start; $row <= $chr_stop; $row++)
{
	my $only_zero = 1;
	
	for(my $col = $chr_start; $col <= $row; $col++)
	{
...
my $out_filename = "../chr".$chr."_non_zero_bins.txt";
...
print "maximize(RowFile, FreqFile, Non_Zero_Rows) :-\n\n";
	...
	for(my $i = 0; $i <= $#non_zero_row_index; $i++)
	{
		...
		{
			print "Chr".$chr."_Row".$non_zero_row_index[$i]."],\n\n";
		}
		else
		{
			print "Chr".$chr."_Row".$non_zero_row_index[$i].", ";
	...
	for(my $i = 0; $i <= $#non_zero_row_index; $i++)
	{
		...
		if($i == $#non_zero_row_index)
		{
			print "Chr".$chr."_Freq".$non_zero_row_index[$i]."],\n\n";
		}
		else
		{
			print "Chr".$chr."_Freq".$non_zero_row_index[$i].", ";
	...
	for(my $i = 0; $i <= $#non_zero_row_index; $i++)
	{
		...
		for(my $col = $chr_start; $col <= $non_zero_row_index[$i]; $col++)
		{	
			##check to see if it is non-zero
		...
		print "\tChr".$chr."_Row".$non_zero_row_index[$i]." :: [0";
	...
	for(my $i = 0; $i <= $#non_zero_row_index; $i++)
	{
		## note: for each row we are only printing the unique interaction frequency values
		my @domain;
		my @unique_values;
		...
		print "\tChr".$chr."_Freq".$non_zero_row_index[$i]." :: [0";
	...
	{	
		...
		{	
			...
			{		
				print "\t((Chr".$chr."_Row".$row." #= ".$nz_col.") and (Chr".$chr."_Freq".$row." #= ".$frequencies[$row][$nz_col].")) or \n";
			}
			## print the "0" case
			print "\t((Chr".$chr."_Row".$row." #= 0) and (Chr".$chr."_Freq".$row." #= 0)),\n";
		}
	}
